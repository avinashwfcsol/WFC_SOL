...
{resume_text}"""
    ...
    for key in api_keys:
        try:
            ...
            # Use lower() for safe comparison
            confidence = model_data.get("confidence_level", "").lower() or computed["confidence_level"].lower()

            # SAFEGUARD: If the AI has low confidence, force a human review instead of auto-rejecting.
            if confidence == "low" and computed["final_decision"] == "REJECTED":
                computed["final_decision"] = "REVIEW"
                computed["decision_reason"] = "AI had low confidence in parsing this resume (possible complex PDF formatting). Human review is strongly recommended."
                computed["recommendation"] = "Review"
